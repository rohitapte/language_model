...
class CharDataGenerator(Sequence):
    def __init__(self,text,tokenizer,timesteps,hidden_size,batch_size=1000,shuffle=True):
        self.text = text
        self.tokenizer = tokenizer
        self.timesteps = timesteps
    ...
    def __getitem__(self,idx):
        ...
        text_encoded = self.tokenizer.texts_to_sequences(text)
        preproc_text = pad_sequences(text_encoded, padding='post', maxlen=self.timesteps)
        preproc_categorical=to_categorical(preproc_text,num_classes=len(self.tokenizer.word_index)+1)
        return preproc_categorical[:,:-1,:],preproc_categorical[:,1:,:]